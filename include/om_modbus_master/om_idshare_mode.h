...
class IdShareMode {
   private:
    ...
    const static int MIN_READ_NUM =
        1;  // 1軸分 初期値は0 ID Shareモード時1~12になる
    const static int MAX_READ_NUM = 12;
    const static int MIN_WRITE_NUM =
        1;  // 1軸分 初期値は0 ID Shareモード時1~12になる
    ...
    // ファンクションコード
    const static int FUNCTION_CODE_WRITE =
        0x10;  // 複数の保持レジスタへの書き込み
    const static int FUNCTION_CODE_READ = 0x03;  // 保持レジスタからの読み出し
    const static int FUNCTION_CODE_READ_WRITE =
        0x17;  // 複数の保持レジスタの読み出し/書き込み
    ...
    const static int MODBUS_MESSAGE_LENGTH_ERROR =
        -8;  // Modbusメッセージ長が仕様範囲外
   ...
   public:
    ...
    int setQuery03h(int query_num, int id, int addr, int num,
                    std::vector<std::vector<char> >& pOut);
    int setQuery10h(int query_num, int id, int addr, int num, int* pVal,
                    std::vector<std::vector<char> >& pOut);
