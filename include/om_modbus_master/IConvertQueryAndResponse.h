...
class IConvertQueryAndResponse {
   public:
    virtual void setRead(int id, int addr, int num,
                         std::vector<std::vector<char> > &pOut) = 0;
    virtual void setWrite(int id, int addr, int num, int *pVal,
                          std::vector<std::vector<char> > &pOut) = 0;
    virtual int setReadAndWrite(int id, int read_addr, int write_addr,
