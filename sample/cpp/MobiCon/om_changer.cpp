...
void init(om_modbus_master::om_query msg, ros::Publisher pub) {
    ...
    pub.publish(
        msg); /* クエリ生成ノードに上記内容を送信。ノードでmsg作成後はドライバに送信
               */
    wait();   /* 処理待ち */
}
...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle n;
    ros::Publisher pub = n.advertise<om_modbus_master::om_query>(
        "om_query1", 1); /* OMノードに送信するための定義 */
    ros::Subscriber sub1 = n.subscribe(
        "om_response1", 1, resCallback); /* レスポンスのコールバック定義 */
    ros::Subscriber sub2 = n.subscribe(
        "om_state1", 1, stateCallback); /* レスポンスのコールバック定義 */

    ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>(
        "odom", 50); /* Odmに送信するための定義 */
    ...
    while (ros::ok()) {
        ...
        msg.read_addr =
            4928; /* 先頭アドレス選択(Dec): 間接参照（０）データ (1340h) */
        msg.read_num = 3; /* 読み込みデータサイズ: 3 (3x32bit) */
        msg.write_addr =
            4960; /* 先頭アドレス選択(Dec): 間接参照（１６データ）(1360h) */
        ...
        geometry_msgs::Quaternion odom_quat =
            tf::createQuaternionMsgFromYaw(odm_th);
