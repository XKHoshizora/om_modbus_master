...
//		wait();					    /* 処理待ち */
		...
		msg.slave_id = 0x01;	    /* 号機選択(Hex): 1 */
		msg.func_code = 2;		    /* ファンクションコード選択: 1(Write) */
		msg.read_addr = 2138;	    /* 先頭アドレス選択(Dec): 検出位置(mm) (085Ah) */
		msg.read_num = 3;		    /* 読み込みデータサイズ: 1 (32bit) */
		msg.write_addr = 1986;	    /* 先頭アドレス選択(Dec): ダイレクト運転データNo.(07C2h) */
		msg.write_num = 3;		    /* 書き込みデータサイズ: 8(8x32bit) */
		...
		odom.twist.twist.linear.x = x_spd/1000;
		odom.twist.twist.linear.y = 0.0;
		odom.twist.twist.angular.z = z_ang/180000*M_PI;
